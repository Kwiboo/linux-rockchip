...
static const struct capture_fmt mp_fmts[] = {
	...
	/* yuv420 */
	{
		...
		.uv_swap = 0,
		.write_format = 0,
		.output_format = ISP32_MI_OUTPUT_YUV400,
	},
};
...
static struct stream_config rkisp_mp_stream_cfg = {
	...
	.max_rsz_width = CIF_ISP_INPUT_W_MAX_V35,
	.max_rsz_height = CIF_ISP_INPUT_H_MAX_V35,
...
static int ldc_config_mi(struct rkisp_stream *stream)
{
	...
	rkisp_unite_set_bits(dev, CTRL_SWS_CFG, ISP33_SW_DITHER_EN,
			     ISP32L_ISP2ENC_CNT_MUX, false);
	val = out_fmt->width;
...
{
	struct rkisp_device *dev = stream->ispdev;
	u32 val = ISP39_LDCV_EN | ISP39_LDCV_FORCE_UPD;

	rkisp_unite_clear_bits(dev, ISP39_LDCV_CTRL, val, false);
}
	...
	if (stream->streaming && !stream->stopping) {
		...
		if (!list_empty(&stream->buf_queue) &&
			...
			if (stream->id == RKISP_STREAM_LDC)
				...
				val = 0;
			if (stream->ops->is_stream_stopped(stream) && !ISP3X_ISP_OUT_LINE(val)) {
				stream->ops->enable_mi(stream);
				stream_self_update(stream);
			}
			if (!stream->ops->is_stream_stopped(stream)) {
