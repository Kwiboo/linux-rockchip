...
{
	struct isp351s_isp_params_cfg *params_rec = params_vdev->isp351s_params + id;
	struct rkisp_device *dev = params_vdev->dev;
	u32 i, data, ctrl;
...
isp_debayer_config(struct rkisp_isp_params_vdev *params_vdev,
		   const struct isp351s_debayer_cfg *arg, u32 id)
{
	u32 i, value;
	...
	value = (arg->g_interp_sharp_strg_max_limit & 0x3F) << 24 | arg->drct_method_thred << 16 |
		...
		(arg->hi_texture_thred & 0x0F) << 4 | !!arg->g_interp_global_sharp_strg_en << 1 |
		!!arg->g_interp_clip_en;
	...
	value = arg->cnr_edge_norm_bit & 0xf;
	isp3_param_write(params_vdev, value, ISP351S_DEBAYER_C_FILTER_EDGE_1, id);

	for (i = 0; i < ISP351S_DEBAYER_MED_NUM / 2; i++) {
		value = ISP_PACK_2SHORT(arg->cnr_med_flt_wgt_min_thred[i * 2],
					arg->cnr_med_flt_wgt_min_thred[i * 2 + 1]);
		isp3_param_write(params_vdev, value, ISP351S_DEBAYER_C_FILTER_MED_THRED0 + i * 4, id);
	}

	value = ISP_PACK_2SHORT(arg->cnr_med_flt_wgt_slope, arg->cnr_med_flt_range_scale);
	isp3_param_write(params_vdev, value, ISP351S_DEBAYER_C_FILTER_MED_0, id);

	value = (arg->cnr_med_flt_index_min & 0xf) |
		(arg->cnr_med_flt_index_max & 0xf) << 4;
	isp3_param_write(params_vdev, value, ISP351S_DEBAYER_C_FILTER_MED_1, id);

	value = ISP_PACK_4BYTE(arg->med_wgt_lpf_coeff0, arg->med_wgt_lpf_coeff1,
			       arg->med_wgt_lpf_coeff2, 0);
	isp3_param_write(params_vdev, value, ISP351S_DEBAYER_C_FILTER_MED_GAUS, id);
}
...
isp_awbgain_config(struct rkisp_isp_params_vdev *params_vdev,
		   const struct isp351s_awb_gain_cfg *arg,
		   enum rkisp_params_type type, u32 id)
{
	struct rkisp_device *dev = params_vdev->dev;
	u32 val;

	if (!arg->gain0_r || !arg->gain0_b ||
	    !arg->gain0_gr || !arg->gain0_gb ||
	    !arg->gain1_r || !arg->gain1_b ||
	    !arg->gain1_gr || !arg->gain1_gb ||
	    !arg->awb1_gain_r || !arg->awb1_gain_b ||
	    !arg->awb1_gain_gr || !arg->awb1_gain_gb) {
		dev_err(dev->dev, "awb gain is zero!\n");
		return;
	}

	if (!dev->is_aiisp_en || type == RKISP_PARAMS_ALL || type == RKISP_PARAMS_LAT) {
		val = (!!(arg->awb1_gain_gb & BIT(16))) |
		      (!!(arg->awb1_gain_gr & BIT(16)) << 4) |
		      (!!(arg->awb1_gain_b & BIT(16)) << 8) |
		      (!!(arg->awb1_gain_r & BIT(16)) << 12);
		isp3_param_write(params_vdev, val, ISP351S_ISP_AWB1_GAIN_MSB, id);
	...
	isp3_param_write(params_vdev,
			 ISP_PACK_2SHORT(arg->gain0_gb, arg->gain0_gr),
			 ISP3X_ISP_AWB_GAIN0_G, id);
	isp3_param_write(params_vdev,
			 ISP_PACK_2SHORT(arg->gain0_b, arg->gain0_r),
			 ISP3X_ISP_AWB_GAIN0_RB, id);

	isp3_param_write(params_vdev,
			 ISP_PACK_2SHORT(arg->gain1_gb, arg->gain1_gr),
			 ISP3X_ISP_AWB_GAIN1_G, id);
	isp3_param_write(params_vdev,
			 ISP_PACK_2SHORT(arg->gain1_b, arg->gain1_r),
...
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
	...
	if (dev->unite_div > ISP_UNITE_DIV1) {
		width = dev->unite.win[id].act_width;
		height = dev->unite.win[id].act_height;
	}
...
isp_rawae_config(struct rkisp_isp_params_vdev *params_vdev,
		 const struct isp351s_rawae_meas_cfg *arg,
		 u32 addr, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	...
	if (ispdev->unite_div > ISP_UNITE_DIV1) {
		width = ispdev->unite.win[id].act_width;
		height = ispdev->unite.win[id].act_height;
	}
...
isp_rawae0_config(struct rkisp_isp_params_vdev *params_vdev,
		  const struct isp351s_rawae_meas_cfg *arg, u32 id)
{
		     ...
		     u32 addr, bool is_check, u32 id)
{
	...
	u8 weight15x15[ISP351S_RAWHISTBIG_WEIGHT_REG_SIZE];
	...
	for (i = 0; i < hist_wnd_num[wnd_num_idx]; i++) {
		for (j = 0; j < hist_wnd_num[wnd_num_idx]; j++) {
			weight15x15[i * ISP351S_RAWHISTBIG_ROW_NUM + j] =
				arg->weight[i * hist_wnd_num[wnd_num_idx] + j];
		}
	}

	for (i = 0; i < (ISP351S_RAWHISTBIG_WEIGHT_REG_SIZE / 5); i++) {
...
isp_aiawb_config(struct rkisp_isp_params_vdev *params_vdev,
		 const struct isp351s_aiawb_meas_cfg *arg, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
...
isp_hdrdrc_config(struct rkisp_isp_params_vdev *params_vdev,
		  const struct isp351s_drc_cfg *arg,
		  enum rkisp_params_type type, u32 id)
{
	u32 i, value, ctrl;

	ctrl = isp3_param_read_cache(params_vdev, ISP3X_DRC_CTRL0, id);
	ctrl &= ISP35_MODULE_EN;
	ctrl |= (arg->gain_shift_bit & 0x7) << 8 |
		!!arg->bf_lp_en << 4 | !!arg->gainx32_en << 3 |
...
isp_hdrdrc_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
	...
	value = isp3_param_read_cache(params_vdev, ISP3X_DRC_CTRL0, id);
...
isp_gic_config(struct rkisp_isp_params_vdev *params_vdev,
	       const struct isp351s_gic_cfg *arg, u32 id)
{
	u32 value, ctrl;
	s32 i;
	...
	value = ISP_PACK_4BYTE(arg->loflt_gr_coef0, arg->loflt_gr_coef1,
			       arg->loflt_gr_coef2, arg->loflt_gr_coef3);
	isp3_param_write(params_vdev, value, ISP351S_GIC_GR_COEF, id);

	value = ISP_PACK_4BYTE(arg->loflt_gb_coef0, arg->loflt_gb_coef1, 0, 0);
	isp3_param_write(params_vdev, value, ISP351S_GIC_GB_COEF, id);

	value = (arg->gaus_coef0 & 0x7f) | (arg->gaus_coef1 & 0x7f) << 8 |
		(arg->gaus_coef2 & 0x7f) << 16;
	isp3_param_write(params_vdev, value, ISP351S_GIC_GAUS_COEF, id);

	value = (arg->bf_coef0 & 0x7f) | (arg->bf_coef1 & 0x7f) << 8 |
		(arg->bf_coef2 & 0x7f) << 16;
	isp3_param_write(params_vdev, value, ISP351S_GIC_BF_COEF, id);

	value = arg->fusion_wgt_slope & 0xfffff;
	isp3_param_write(params_vdev, value, ISP351S_GIC_WGT_SLOPE, id);

	value = (arg->gaus_alpha & 0x7f) | (arg->bf_out_alpha & 0x7f) << 8;
	isp3_param_write(params_vdev, value, ISP351S_GIC_ALPHA, id);

	value = (arg->bf_wet_min & 0x3ff) | (arg->bf_wet_scale & 0x3fff) << 16;
	isp3_param_write(params_vdev, value, ISP351S_GIC_BF_PARA0, id);

	value = (arg->r_noise_strg & 0x3ff) | (arg->b_noise_strg & 0x3ff) << 16;
	isp3_param_write(params_vdev, value, ISP351S_GIC_BF_PARA1, id);

	value = arg->rat2wgt_min_thed & 0x3fff;
	isp3_param_write(params_vdev, value, ISP351S_GIC_BF_PARA2, id);

	for (i = 0; i < ISP351S_GIC_CURVE_NUM / 4; i++) {
		value = (arg->curve_idx[i * 4] & 0x1f) |
			(arg->curve_idx[i * 4 + 1] & 0x1f) << 8 |
			(arg->curve_idx[i * 4 + 2] & 0x1f) << 16 |
			(arg->curve_idx[i * 4 + 3] & 0x1f) << 24;
		isp3_param_write(params_vdev, value, ISP351S_GIC_CURVE_IDX0 + i * 4, id);
	}
	value = (arg->curve_idx[i * 4] & 0x1f) |
		(arg->curve_idx[i * 4 + 1] & 0x1f) << 8 |
		(arg->curve_idx[i * 4 + 2] & 0x1f) << 16;
	isp3_param_write(params_vdev, value, ISP351S_GIC_CURVE_IDX2, id);

	for (i = 0; i < ISP351S_GIC_THRED_NUM; i++) {
		value = arg->fusion_min_thred[i] & 0xfffff;
		isp3_param_write(params_vdev, value, ISP351S_GIC_FUSION_MIN0 + i * 4, id);

		value = arg->luma2soft_thred[i] & 0xfffff;
		isp3_param_write(params_vdev, value, ISP351S_GIC_SOFT_THED0 + i * 4, id);
	}

	for (i = 0; i < ISP351S_GIC_SIGMA_NUM; i++) {
		value = arg->r_sigma_inv[i] & 0xfffffff;
		isp3_param_write(params_vdev, value, ISP351S_GIC_R_SIGMA_INV0 + i * 4, id);

		value = arg->b_sigma_inv[i] & 0xfffffff;
		isp3_param_write(params_vdev, value, ISP351S_GIC_B_SIGMA_INV0 + i * 4, id);
	}
}
...
isp_enh_cfg_sram(struct rkisp_isp_params_vdev *params_vdev,
		 const struct isp351s_enh_cfg *arg, bool is_check, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	u32 ctrl = isp3_param_read(params_vdev, ISP33_ENH_CTRL, id);
	u32 i, num, val;
	...
	val = ISP351S_ENH_IIR_RW_NUM(priv->enh_scl_16bit_num[id]) | ISP351S_ENH_IIR_WR_CLEAR;
	isp3_param_write_direct(params_vdev, val, ISP33_ENH_IIR_RW);
	num = (priv->enh_scl_16bit_num[id] + 1) / 2;
	if (num > ISP351S_ENH_IIR_DATA_MAX)
		num = ISP351S_ENH_IIR_DATA_MAX;
	for (i = 0; i < num; i++)
		isp3_param_write_direct(params_vdev, arg->iir_data[i], ISP33_ENH_IIR0);
}

static void
isp_enh_config(struct rkisp_isp_params_vdev *params_vdev,
	       const struct isp351s_enh_cfg *arg, u32 id)
{
	struct isp351s_isp_params_cfg *params_rec = params_vdev->isp351s_params + id;
	struct isp351s_enh_cfg *arg_rec = &params_rec->others.enh_cfg;
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
	struct v4l2_rect *out_crop = &dev->isp_sdev.out_crop;
	u32 w = out_crop->width, h = out_crop->height;
	u32 i, value, ctrl, het_aliquant, scl_h, scl_w;

	if (dev->unite_div > ISP_UNITE_DIV1) {
		w = dev->unite.win[id].act_width;
		h = dev->unite.win[id].act_height;
	}
	scl_w = (w + 127) / 128;
	scl_h = (h + 128) / 129;
	value = scl_w * ((scl_h + 1) / 2);
	priv->enh_scl_16bit_num[id] = value - !(value & BIT(0));
	het_aliquant = h % 3;
	...
	value = (arg->iir_inv_sigma & 0x1ff) |
		(arg->iir_soft_thed & 0x1f) << 16 |
		(arg->iir_cur_wgt & 0x3f) << 24;
	...
	value = (arg->blf5_inv_sigma & 0x1ff) | (arg->blf5_cur_wgt & 0x3f) << 16;
...
isp_hist_cfg_sram(struct rkisp_isp_params_vdev *params_vdev,
		  const struct isp351s_hist_cfg *arg, bool is_check, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	u32 i, val, ctrl = isp3_param_read(params_vdev, ISP33_HIST_CTRL, id);
	...
	val = ISP351S_HIST_IIR_WR_CLEAR;
	isp3_param_write_direct(params_vdev, val, ISP33_HIST_RW);
	for (i = 0; i < priv->hist_blk_num[id]; i++)
		isp3_param_write_direct(params_vdev, arg->iir_data[i], ISP33_HIST_IIR0);
}

static void
isp_hist_config(struct rkisp_isp_params_vdev *params_vdev,
		const struct isp351s_hist_cfg *arg, u32 id)
{
	struct rkisp_device *dev = params_vdev->dev;
	struct v4l2_rect *out_crop = &dev->isp_sdev.out_crop;
	struct isp351s_isp_params_cfg *params_rec = params_vdev->isp351s_params + id;
	struct isp351s_hist_cfg *arg_rec = &params_rec->others.hist_cfg;
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	u32 w = out_crop->width, h = out_crop->height;
	...
	value = (arg->flt_offset & 0x7f) | (arg->flt_sigma & 0x7f) << 8;
	isp3_param_write(params_vdev, value, ISP351S_HIST_MAP_FLT, id);

	value = (arg->gain_min & 0x7f) | (arg->gain_max & 0x7ff) << 8;
	isp3_param_write(params_vdev, value, ISP351S_HIST_GAIN, id);

	value = ISP_PACK_4BYTE(arg->coef_max0, arg->coef_max1,
			       arg->coef_max2, arg->coef_max3);
	isp3_param_write(params_vdev, value, ISP351S_HIST_COEF_MAX, id);

	value = ISP_PACK_4BYTE(arg->coef_min0, arg->coef_min1,
			       arg->coef_min2, arg->coef_min3);
	isp3_param_write(params_vdev, value, ISP351S_HIST_COEF_MIN, id);

	value = arg->saturate_scale;
	isp3_param_write(params_vdev, value, ISP33_HIST_UV_SCL, id);

	thumb_row = arg->thumb_row > ISP351S_HIST_THUMB_ROW_MAX ?
		    ISP351S_HIST_THUMB_ROW_MAX : arg->thumb_row & ~1;
	thumb_col = arg->thumb_col > ISP351S_HIST_THUMB_COL_MAX ?
		    ISP351S_HIST_THUMB_COL_MAX : arg->thumb_col & ~1;
	blk_het = ALIGN(h / thumb_row, 2);
	blk_wid = ALIGN(w / thumb_col, 2);
	priv->hist_blk_num[id] = thumb_row * thumb_col * 5;
	if (priv->hist_blk_num[id] > ISP351S_HIST_IIR_DATA_MAX)
		priv->hist_blk_num[id] = ISP351S_HIST_IIR_DATA_MAX;
	...
	if (dev->hw_dev->is_single && arg->iir_wr)
		...
		memcpy(arg_rec, arg, sizeof(struct isp351s_hist_cfg));
}
		...
		const struct isp39_ldch_cfg *arg, u32 id)
{
	...
	priv->ldch_out_hsize = arg->out_hsize;
		...
		const struct isp39_ldcv_cfg *arg, u32 id)
{
	...
	priv->ldcv_out_vsize = arg->out_vsize;

	value = isp3_param_read(params_vdev, ISP39_LDCV_CTRL, id);
	value &= (ISP39_LDCV_EN | ISP39_LDCV_OUTPUT_YUV420 | ISP39_LDCV_UV_SWAP);
	...
	for (i = 0; i < ISP39_MESH_BUF_NUM; i++) {
		if (!priv->buf_ldcv[id][i].mem_priv)
			continue;
		if (arg->buf_fd == priv->buf_ldcv[id][i].dma_fd)
			break;
	}
	...
	if (!priv->buf_ldcv[id][i].vaddr) {
		dev_err(dev->dev, "no ldcv buffer allocated\n");
		return;
	}

	buf_idx = priv->buf_ldcv_idx[id];
	head = priv->buf_ldcv[id][buf_idx].vaddr;
	head->stat = MESH_BUF_INIT;

	buf_idx = i;
	head = priv->buf_ldcv[id][buf_idx].vaddr;
	head->stat = MESH_BUF_CHIPINUSE;
	priv->buf_ldcv_idx[id] = buf_idx;
	rkisp_prepare_buffer(dev, &priv->buf_ldcv[id][buf_idx]);
	value = priv->buf_ldcv[id][buf_idx].dma_addr + head->data_oft;
...
isp_ldcv_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
	...
	if (en) {
		buf_idx = priv->buf_ldcv_idx[id];
		if (!stream->streaming || !priv->buf_ldcv[id][buf_idx].vaddr) {
			dev_err(dev->dev, "output stream en:%d, ldcv map buf:%p\n",
				stream->streaming, priv->buf_ldcv[id][buf_idx].vaddr);
			goto end;
		}
		if (stream->out_fmt.width != priv->ldch_out_hsize ||
		    stream->out_fmt.height != priv->ldcv_out_vsize) {
			dev_err(dev->dev, "stream output %dx%d, but ldch:%d ldcv:%d\n",
				stream->out_fmt.width, stream->out_fmt.height,
				priv->ldch_out_hsize, priv->ldcv_out_vsize);
			goto end;
		}
		value |= ISP39_LDCV_EN;
		if (!(dev->isp_state & ISP_START))
			value |= ISP39_LDCV_FORCE_UPD;
	}
end:
	isp3_param_write(params_vdev, value, ISP39_LDCV_CTRL, id);
}
...
isp_ynr_config(struct rkisp_isp_params_vdev *params_vdev,
	       const struct isp351s_ynr_cfg *arg, u32 id)
{
	u32 i, value;
	...
	value = (!!arg->hi_spnr_filt_noise_limit_en) |
		(arg->hi_spnr_filt_noise_min_limit_scale & 0x3ff) << 2 |
		(arg->hi_spnr_filt_noise_max_limit_scale & 0x3ff) << 12 |
		(arg->hi_spnr_filt_diff_offset_scale & 0x3ff) << 22;
	isp3_param_write(params_vdev, value, ISP351S_YNR_HI_SIGMA_LIMIT, id);
	...
	value = (!!arg->mi_spnr_filt_edge_adjust_en) |
		(arg->mi_spnr_filt_edge_offset & 0x3ff) << 4 |
		(arg->mi_spnr_filt_edge_scale & 0x1f) << 16;
	isp3_param_write(params_vdev, value, ISP351S_YNR_MI_EDGE_ADJUST, id);

	value = (!!arg->mi_spnr_tex_adjust_en) |
		(arg->mi_spnr_tex_min_weight & 0x7ff) << 1 |
		arg->mi_spnr_tex_scale << 12 |
		(arg->mi_spnr_tex_thred & 0x3ff) << 20;
	isp3_param_write(params_vdev, value, ISP351S_YNR_MI_TEXT_ADJUST, id);
...
isp_bay3d_config(struct rkisp_isp_params_vdev *params_vdev,
		 const struct isp351s_bay3d_cfg *arg, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
	struct rkisp_stream *stream = &dev->cap_dev.stream[RKISP_STREAM_LDC];
	...
	value = (arg->transf_mode_offset1 & 0xfff) | (arg->transf_mode_ob & 0x3ff) << 16;
	isp3_param_write(params_vdev, value, ISP351S_BAY3D_TRANS2, id);
	...
	if (stream->streaming) {
		dev_err(dev->dev, "no using b3dldc when ldc enable\n");
		return;
	}
	...
	isp3_param_write(params_vdev, value, ISP39_LDCV_LAST_OFFSET, id);
	...
	if (arg->btnr_ldc_en) {
		...
		isp3_param_write(params_vdev, value, ISP32L_IRLDCV_RD_BASE, id);
		value = priv->b3dldc_hsize;
		isp3_param_write(params_vdev, value, ISP32L_IRLDCV_RD_H_WSIZE, id);
		value = priv->b3dldcv_vsize;
		isp3_param_write(params_vdev, value, ISP32L_IRLDCV_RD_V_SIZE, id);
		ctrl |= ISP351S_B3DMCV_EN | ISP351S_B3DMCV_MODE;
	}
	isp3_param_write(params_vdev, ctrl, ISP39_LDCV_CTRL, id);
	if (dev->hw_dev->is_single) {
		value = ISP351S_RXIRLDC_FORCE_UPD | ISP351S_RXB3DMC_FORCE_UPD;
...
isp_bay3d_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
	u32 value, ctrl, b3dldc;

	ctrl = isp3_param_read_cache(params_vdev, ISP33_BAY3D_CTRL0, id);
	if (en == !!(ctrl & ISP35_MODULE_EN))
		return;

	b3dldc = isp3_param_read_cache(params_vdev, ISP39_LDCV_CTRL, id);
	if (en) {
		...
		value = priv->bay3d_iir_stride[id];
		...
		if (priv->bay3d_iir_rw_fmt == 3)
			isp3_param_write(params_vdev, value, ISP39_LDCV_WR_STRIDE, id);

		priv->bay3d_iir_idx = 0;
		priv->bay3d_iir_cur_idx = 0;
		value = priv->bay3d_iir_size[id];
		isp3_param_write(params_vdev, value, ISP3X_MI_BAY3D_IIR_WR_SIZE, id);
		value = priv->buf_bay3d_iir[0].dma_addr + priv->bay3d_iir_offs[id];
		isp3_param_write(params_vdev, value, ISP3X_MI_BAY3D_IIR_RD_BASE, id);
		if (priv->bay3d_iir_rw_fmt == 3) {
			isp3_param_write(params_vdev, priv->bay3d_iir_pk_stride[id], ISP3X_MI_BAY3D_IIR_WR_LENGTH, id);

			isp3_param_write(params_vdev, value, ISP39_LDCV_WR_ADDR, id);
			if (b3dldc & ISP351S_B3DMCV_MODE) {
				b3dldc |= ISP351S_B3DMCV_FORCE_UPD;
				isp3_param_write(params_vdev, b3dldc, ISP39_LDCV_CTRL, id);
			}
			value += priv->bay3d_iir_pk_offs[id];
		}
		isp3_param_write(params_vdev, value, ISP3X_MI_BAY3D_IIR_WR_BASE, id);
		if (priv->buf_aiisp[0].mem_priv) {
			priv->aiisp_cur_idx = 0;
			value = priv->buf_aiisp[0].dma_addr;
		...
		value = priv->bay3d_ds_size[id];
		isp3_param_write(params_vdev, value, ISP3X_MI_BAY3D_DS_WR_SIZE, id);
		value = priv->buf_bay3d_ds[0].dma_addr + priv->bay3d_ds_offs[id];
		...
		value = priv->buf_bay3d_wgt[0].dma_addr + priv->bay3d_wgt_offs[id];
		...
		if (priv->buf_gain[0].mem_priv) {
			...
			if (!params_vdev->is_hdr) {
				...
				isp3_param_write(params_vdev, 0, ISP3X_MI_RAW0_RD_LENGTH, id);
			} else {
				isp3_param_write(params_vdev, value, ISP35_B3DLDCH_RD_BASE, id);
				isp3_param_write(params_vdev, 0, ISP35_B3DLDCH_RD_LENGTH, id);
			}
			priv->gain_cur_idx = 0;
		}
		...
		isp3_param_write(params_vdev, ctrl, ISP33_BAY3D_CTRL0, id);
		if (b3dldc & ISP351S_B3DMCV_MODE) {
			b3dldc &= ~(ISP351S_B3DMCV_FORCE_UPD | ISP351S_B3DMCV_EN);
			isp3_param_write(params_vdev, b3dldc, ISP39_LDCV_CTRL, id);
		...
		value |= ISP35_W3A_B3DNROUT_ILG_BYPASS;
		isp3_param_write(params_vdev, value, ISP39_W3A_CTRL0, id);
		isp3_param_clear_bits(params_vdev, ISP33_BAY3D_CTRL1, 1, id);
	}
}
...
{
	struct isp33_cac_cfg *arg_rec = &params_vdev->isp351s_params->others.cac_cfg;
	u32 i, val, ctrl;
...
isp_ai_config(struct rkisp_isp_params_vdev *params_vdev,
	      const struct isp351s_ai_cfg *arg, u32 id)
{
	struct isp351s_isp_params_cfg *params_rec = params_vdev->isp351s_params + id;
	struct isp351s_ai_cfg *arg_rec = &params_rec->others.ai_cfg;
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	u32 i, val;
...
isp_ai_enable(struct rkisp_isp_params_vdev *params_vdev, bool en, u32 id)
{
	...
	ctrl &= ~(ISP35_AIISP_ST | ISP35_AIPRE_IIR2DDR_EN | ISP35_AIPRE_GIAN2DDR_EN |
		  ISP35_AIPRE_IIR_EN | ISP35_AIPRE_GAIN_EN);
	if (en) {
		if (priv->buf_aipre[0].mem_priv) {
			priv->aipre_idx = 0;
			priv->aipre_cur_idx = 0;
			val = priv->buf_aipre[0].dma_addr;
			isp3_param_write(params_vdev, val, ISP35_AI_PRE_NL_WR_BASE, id);
			val = priv->aipre_stride;
			isp3_param_write(params_vdev, val, ISP35_AI_PRE_NL_WR_STRIDE, id);
			ctrl |= ISP35_AIPRE_IIR_EN | ISP35_AIPRE_IIR2DDR_EN;
		}
...
void __isp_isr_other_config(struct rkisp_isp_params_vdev *params_vdev,
			    const struct isp351s_isp_params_cfg *new_params,
			    enum rkisp_params_type type, u32 id)
{
	struct rkisp_device *dev = params_vdev->dev;
...
void __isp_isr_meas_config(struct rkisp_isp_params_vdev *params_vdev,
			   struct isp351s_isp_params_cfg *new_params,
			   enum rkisp_params_type type, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
...
static bool
rkisp_params_check_bigmode_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
	struct rkisp_device *dev = params_vdev->dev;
...
static void
rkisp_params_first_cfg_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
	struct rkisp_device *dev = params_vdev->dev;
	struct isp351s_isp_params_cfg *params = params_vdev->isp351s_params;
	struct rkisp_stream *stream = &dev->cap_dev.stream[RKISP_STREAM_LDC];
	unsigned long flags = 0;
	int i;

	rkisp_params_check_bigmode_v351s(params_vdev);
	...
	for (i = 0; i < dev->unite_div; i++) {
		...
		if (stream->streaming && !(module_en_update & ISP39_MODULE_LDCV)) {
			dev_err(dev->dev, "id:%d ldcv no enable for first iq\n", i);
...
static void rkisp_save_first_param_v351s(struct rkisp_isp_params_vdev *params_vdev, void *param)
{
	u32 size = params_vdev->vdev_fmt.fmt.meta.buffersize;

	memcpy(params_vdev->isp35_params, param, size);
}

static void rkisp_clear_first_param_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
...
static void
rkisp_get_param_size_v351s(struct rkisp_isp_params_vdev *params_vdev,
			   unsigned int sizes[])
{
	u32 mult = params_vdev->dev->unite_div;

	sizes[0] = sizeof(struct isp351s_isp_params_cfg) * mult;
	params_vdev->vdev_fmt.fmt.meta.buffersize = sizes[0];
}

static void
rkisp_params_get_meshbuf_inf_v351s(struct rkisp_isp_params_vdev *params_vdev,
				   void *meshbuf_inf)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
...
static int
rkisp_params_set_meshbuf_size_v351s(struct rkisp_isp_params_vdev *params_vdev,
				    void *size)
{
...
static void
rkisp_params_free_meshbuf_v351s(struct rkisp_isp_params_vdev *params_vdev,
				u64 module_id)
{
	int id;
...
static int
rkisp_params_info2ddr_cfg_v351s(struct rkisp_isp_params_vdev *params_vdev, void *arg)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
...
static int
rkisp_params_init_bnr_buf_v351s(struct rkisp_isp_params_vdev *params_vdev,
				struct rkisp_bnr_buf_info *bnrbuf)
{
	...
	struct rkisp_dummy_buffer *buf;
	u32 iir_rw_fmt, size, w16, w32, w128, iir_size = 0;
	int ret, i, j, idx, cnt, total_size, w, h, stride = 0;

	INIT_LIST_HEAD(&priv->iir_list);
	INIT_LIST_HEAD(&priv->gain_list);
	INIT_LIST_HEAD(&priv->aipre_list);

	iir_rw_fmt = bnrbuf->u.v351s.iir_rw_fmt;
	total_size = 0;
	for (i = 0; i < dev->unite.v_div; i++) {
		for (j = 0; j < dev->unite.h_div; j++) {
			...
			w32 = ALIGN(w, 32);
			w128 = ALIGN(w, 128);
			priv->bay3d_iir_stride[idx] = 0;
			priv->bay3d_iir_pk_stride[idx] = 0;
			priv->bay3d_iir_pk_offs[idx] = 0;
			priv->bay3d_iir_offs[idx] = 0;
			switch (iir_rw_fmt) {
			case 0:
			...
			case 4:
				...
				priv->bay3d_iir_stride[idx] = stride;
				break;
			case 3:
				/* pk_wr */
				stride = ALIGN(w128 / 8 * 2, 16);
				priv->bay3d_iir_pk_stride[idx] = stride;
				/* iir_rw + pk_rd */
				stride = ALIGN((w32 + w128 / 8) * 2, 16);
				priv->bay3d_iir_stride[idx] = stride;
				/* iir_rw + pk_rd + pk_wr */
				priv->bay3d_iir_pk_offs[idx] = stride * h;
				size = (priv->bay3d_iir_stride[idx] + priv->bay3d_iir_pk_stride[idx]) * h;
				break;
			default:
			...
			priv->bay3d_iir_size[idx] = size;
			priv->bay3d_iir_offs[idx] = total_size;
			total_size += size;
		}
	}
	if (dev->unite_div > ISP_UNITE_DIV1) {
		bnrbuf->u.v351s.aibnr_l2 = 0;
		stride = 0;
	}
	...
	bnrbuf->iir.buf_size = total_size;
	...
	for (i = 0; i < dev->unite.v_div; i++) {
		for (j = 0; j < dev->unite.h_div; j++) {
			...
			priv->bay3d_ds_size[idx] = size;
			priv->bay3d_ds_offs[idx] = total_size;
			total_size += size;
		}
	}
	if (dev->unite_div > ISP_UNITE_DIV1)
		stride = 0;
	cnt = bnrbuf->u.v351s.ds.buf_cnt;
	if (cnt >= RKISP_BUFFER_MAX)
		cnt = RKISP_BUFFER_MAX - 1;
	for (i = 0; i < cnt; i++) {
		...
		buf->index = i;
		bnrbuf->u.v351s.ds.buf_fd[i] = buf->dma_fd;
	}
	priv->bay3d_ds_cnt = cnt;
	bnrbuf->u.v351s.ds.buf_cnt = cnt;
	bnrbuf->u.v351s.ds.buf_size = total_size;
	bnrbuf->u.v351s.ds.buf_stride = stride;
	...
	for (i = 0; i < dev->unite.v_div; i++) {
		for (j = 0; j < dev->unite.h_div; j++) {
			...
			priv->bay3d_wgt_size[idx] = size;
			priv->bay3d_wgt_offs[idx] = total_size;
			total_size += size;
		}
	}
	if (dev->unite_div > ISP_UNITE_DIV1)
	...
	for (i = 0; i < cnt; i++) {
		...
		buf->index = i;
		bnrbuf->u.v351s.wgt.buf_fd[i] = buf->dma_fd;
	}
	priv->bay3d_wgt_cnt = cnt;
	bnrbuf->u.v351s.wgt.buf_cnt = cnt;
	bnrbuf->u.v351s.wgt.buf_size = total_size;
	bnrbuf->u.v351s.wgt.buf_stride = stride;

	w = isp_sdev->out_crop.width;
	h = isp_sdev->out_crop.height;
	w32 = ALIGN(w, 32);

	stride = w32 * 2;
	size = stride * h;
	cnt = bnrbuf->u.v351s.aiisp.buf_cnt;
	if (cnt >= RKISP_BUFFER_MAX)
		cnt = RKISP_BUFFER_MAX - 1;
	for (i = 0; i < cnt && iir_size; i++) {
		...
		buf->index = i;
		bnrbuf->u.v351s.aiisp.buf_fd[i] = buf->dma_fd;
	}
	priv->aiisp_cnt = cnt;
	bnrbuf->u.v351s.aiisp.buf_cnt = cnt;
	bnrbuf->u.v351s.aiisp.buf_size = size;
	bnrbuf->u.v351s.aiisp.buf_stride = stride;

	if (bnrbuf->u.v351s.gain_mode)
		stride = w / 8;
	else
		stride = w / 4;
	size = ALIGN(stride * h, 16);
	priv->gain_size = size;
	cnt = bnrbuf->u.v351s.gain.buf_cnt;
	if (cnt >= RKISP_BUFFER_MAX)
		cnt = RKISP_BUFFER_MAX - 1;
	for (i = 0; i < cnt; i++) {
		buf = &priv->buf_gain[i];
		...
		buf->index = i;
		bnrbuf->u.v351s.gain.buf_fd[i] = buf->dma_fd;
	}
	priv->gain_cnt = cnt;
	bnrbuf->u.v351s.gain.buf_cnt = cnt;
	bnrbuf->u.v351s.gain.buf_size = size;
	bnrbuf->u.v351s.gain.buf_stride = stride;

	stride = ALIGN(w, 16);
	priv->aipre_stride = stride;
	size = ALIGN(stride * h, 16);
	cnt = bnrbuf->u.v351s.aipre.buf_cnt;
	if (cnt >= RKISP_BUFFER_MAX)
		cnt = RKISP_BUFFER_MAX - 1;
	for (i = 0; i < cnt; i++) {
		buf = &priv->buf_aipre[i];
		buf->size = size;
		buf->stride = stride;
		buf->is_need_dbuf = true;
		buf->is_need_dmafd = true;
		ret = rkisp_alloc_buffer(dev, buf);
		if (ret) {
			dev_err(dev->dev, "alloc aipre buf%d fail:%d\n", i, ret);
			goto err_aipre;
		}
		if (!i)
			priv->pbuf_aipre = buf;
		else
			list_add_tail(&buf->queue, &priv->aipre_list);
		buf->index = i;
		bnrbuf->u.v351s.aipre.buf_fd[i] = buf->dma_fd;
	}
	priv->aipre_cnt = cnt;
	bnrbuf->u.v351s.aipre.buf_cnt = cnt;
	bnrbuf->u.v351s.aipre.buf_size = size;
	bnrbuf->u.v351s.aipre.buf_stride = stride;

	priv->bay3d_iir_rw_fmt = iir_rw_fmt;

	if (bnrbuf->u.v351s.aibnr_l2) {
		...
		buf->size = bnrbuf->u.v351s.ds.buf_size;
		ret = rkisp_alloc_buffer(dev, buf);
		if (ret) {
		...
		buf->size = bnrbuf->u.v351s.wgt.buf_size;
		ret = rkisp_alloc_buffer(dev, buf);
		if (ret) {
...
err_aipre:
	...
	priv->aipre_cnt = 0;
	bnrbuf->u.v351s.aipre.buf_cnt = 0;
	bnrbuf->u.v351s.aipre.buf_size = 0;
...
err_gain:
	...
	priv->gain_cnt = 0;
	bnrbuf->u.v351s.gain.buf_cnt = 0;
	bnrbuf->u.v351s.gain.buf_size = 0;
...
err_aiisp:
	...
	priv->aiisp_cnt = 0;
	bnrbuf->u.v351s.aiisp.buf_cnt = 0;
	bnrbuf->u.v351s.aiisp.buf_size = 0;
...
err_wgt:
	...
	bnrbuf->u.v351s.wgt.buf_cnt = 0;
	bnrbuf->u.v351s.wgt.buf_size = 0;
...
err_ds:
	...
	priv->bay3d_ds_cnt = 0;
	bnrbuf->u.v351s.ds.buf_cnt = 0;
	bnrbuf->u.v351s.ds.buf_size = 0;
...
static int
rkisp_params_get_aiawb_buffd_v351s(struct rkisp_isp_params_vdev *params_vdev, void *arg)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
...
static void
rkisp_params_stream_stop_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
	int i;
	...
	for (i = 0; i < priv->aipre_cnt; i++)
		rkisp_free_buffer(dev, &priv->buf_aipre[i]);
	priv->aipre_cnt = 0;
...
static void
rkisp_params_fop_release_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
	int id;
	...
	for (id = 0; id < params_vdev->dev->unite_div; id++) {
		...
		rkisp_deinit_mesh_buf(params_vdev, ISP35_MODULE_BAY3D, id);
	}
}

static void
rkisp_params_disable_isp_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
	int i;

	params_vdev->isp351s_params->module_ens = 0;
	params_vdev->isp351s_params->module_en_update = ~ISP35_MODULE_FORCE;
	for (i = 0; i < params_vdev->dev->unite_div; i++) {
		__isp_isr_other_en(params_vdev, params_vdev->isp351s_params, RKISP_PARAMS_ALL, i);
		__isp_isr_meas_en(params_vdev, params_vdev->isp351s_params, RKISP_PARAMS_ALL, i);
	}
}

static void
module_data_abandon(struct rkisp_isp_params_vdev *params_vdev,
		    struct isp351s_isp_params_cfg *params, u32 id)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct isp2x_mesh_head *mesh_head;
	int i;
	...
	if (params->module_cfg_update & ISP39_MODULE_LDCV) {
		...
		for (i = 0; i < ISP39_MESH_BUF_NUM; i++) {
			if (priv->buf_ldcv[id][i].vaddr &&
			    arg->buf_fd == priv->buf_ldcv[id][i].dma_fd) {
				mesh_head = priv->buf_ldcv[id][i].vaddr;
				if (mesh_head->stat == MESH_BUF_WAIT2CHIP)
					mesh_head->stat = MESH_BUF_INIT;
				break;
			}
	...
	if (params->module_cfg_update & ISP35_MODULE_BAY3D) {
		const struct isp351s_bay3d_cfg *arg = &params->others.bay3d_cfg;
...
static void
rkisp_params_cfg_latter_v351s(struct rkisp_isp_params_vdev *params_vdev, u32 frame_id)
{
	struct rkisp_device *dev = params_vdev->dev;
	struct isp351s_isp_params_cfg *new_params = NULL;
		...
		if (new_params->frame_id < frame_id) {
			...
			for (i = 0; i < dev->unite_div; i++) {
				...
				if (new_params->module_cfg_update &
				    (ISP35_MODULE_LDCH | ISP35_MODULE_BAY3D | ISP39_MODULE_LDCV))
...
static void
rkisp_params_cfg_v351s(struct rkisp_isp_params_vdev *params_vdev,
		     u32 frame_id, enum rkisp_params_type type)
{
	struct rkisp_device *dev = params_vdev->dev;
	struct isp351s_isp_params_cfg *params_rec, *new_params = NULL;
	struct rkisp_buffer *cur_buf = NULL;
	unsigned long flags = 0;
	int i;

	if (type == RKISP_PARAMS_LAT) {
		rkisp_params_cfg_latter_v351s(params_vdev, frame_id);
		return;
	}
	...
	params_rec = params_vdev->isp351s_params;
	new_params = cur_buf->vaddr[0];
	for (i = 0; i < dev->unite_div; i++) {
		__isp_isr_meas_config(params_vdev, new_params, type, i);
		__isp_isr_other_config(params_vdev, new_params, type, i);
...
rkisp_params_clear_fstflg(struct rkisp_isp_params_vdev *params_vdev)
{
	...
	value &= (ISP3X_YNR_FST_FRAME | ISP33_YHIST_FST_FRAME |
		  ...
		  ISP33_ENH_FST_FRAME | ISP32_SHP_FST_FRAME);
...
rkisp_params_aiisp_update_buf(struct rkisp_isp_params_vdev *params_vdev)
{
	...
	priv->pbuf_aipre = NULL;
	if (!list_empty(&priv->aipre_list)) {
		priv->pbuf_aipre = list_first_entry(&priv->aipre_list,
					struct rkisp_dummy_buffer, queue);
		list_del(&priv->pbuf_aipre->queue);

		val = priv->pbuf_aipre->dma_addr;
		isp3_param_write(params_vdev, val, ISP35_AI_PRE_GAIN_WR_BASE, 0);
		priv->aipre_cur_idx = priv->pbuf_aipre->index;
	...
	v4l2_dbg(3, rkisp_debug, &params_vdev->dev->v4l2_dev,
		 "aiisp_update %x:%x %x:%x %x:%x, iir:%x gain:%x aipre:%x\n",
		 ...
		 priv->pbuf_aipre ? (u32)priv->pbuf_aipre->dma_addr : 0);
	if (!priv->pbuf_gain_wr || !priv->pbuf_aipre || !priv->pbuf_bay3d_iir) {
		...
		if (priv->pbuf_aipre) {
			list_add_tail(&priv->pbuf_aipre->queue, &priv->aipre_list);
			priv->pbuf_aipre = NULL;
		}
	}
...
static void
rkisp_params_aiisp_event_v351s(struct rkisp_isp_params_vdev *params_vdev, u32 irq)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct rkisp_device *dev = params_vdev->dev;
	struct rkisp_dummy_buffer *buf = NULL;
	...
	if (irq & ISP3X_OUT_FRM_QUARTER) {
		...
		if (priv->pbuf_gain_rd && priv->gain_cnt > 1) {
		...
		spin_lock_irqsave(&priv->buf_lock, lock_flags);
		if (dev->is_aiisp_en &&
		    (!priv->pbuf_bay3d_iir || !priv->pbuf_gain_wr || !priv->pbuf_aipre)) {
			if (priv->pbuf_bay3d_iir && priv->bay3d_iir_cnt > 1) {
				list_add_tail(&priv->pbuf_bay3d_iir->queue, &priv->iir_list);
				priv->pbuf_bay3d_iir = NULL;
			}
			if (priv->pbuf_gain_wr && priv->gain_cnt > 1) {
				list_add_tail(&priv->pbuf_gain_wr->queue, &priv->gain_list);
				priv->pbuf_gain_wr = NULL;
			}
			if (priv->pbuf_aipre && priv->aipre_cnt > 1) {
				list_add_tail(&priv->pbuf_aipre->queue, &priv->aipre_list);
				priv->pbuf_aipre = NULL;
			}
	...
	v4l2_dbg(3, rkisp_debug, &dev->v4l2_dev,
		 "%s seq:%d height:%d idx(iir:%d gain:%d aipre:%d aiisp:%d)\n",
		 ev.id ? "isp_be" : "isp_fe", ev_info->sequence, ev_info->height,
		 ev_info->iir_index, ev_info->gain_index,
		 ev_info->aipre_gain_index, ev_info->aiisp_index);
}

static int
rkisp_params_aiisp_start_v351s(struct rkisp_isp_params_vdev *params_vdev,
			       struct rkisp_aiisp_st *st)
{
	...
	if (!dev->is_aiisp_en && !dev->is_aiisp_stop)
		return -EINVAL;
	v4l2_dbg(3, rkisp_debug, &dev->v4l2_dev,
		 "isp_be input seq:%d idx(iir:%d gain:%d aipre:%d aiisp:%d)\n",
		 seq, st->iir_index, st->gain_index,
	...
	if (st->gain_index < 0 || st->gain_index >= priv->gain_cnt ||
	    ...
	    st->aipre_gain_index >= priv->aipre_cnt) {
		dev_err(dev->dev, "%s seq:%d error, aiisp(%d cnt:%d)\n"
			"iir(%d cnt:%d) gain(%d cnt:%d) aipre(%d cnt:%d)\n",
	...
	if (st->aiisp_index >= 0) {
		...
		val = priv->bay3d_iir_stride[0];
		rkisp_write(dev, ISP3X_MI_DBR_RD_LENGTH, val, false);
	}
	priv->aiisp_cur_idx = st->aiisp_index;

	if (priv->bay3d_iir_cnt > 1) {
	...
	if (st->aipre_gain_index >= 0 && priv->aipre_cnt > 1) {
		buf = &priv->buf_aipre[st->aipre_gain_index];
		list_for_each_entry(buf_tmp, &priv->aipre_list, queue) {
			if (buf_tmp == buf) {
	...
	if (dev->is_aiisp_stop) {
		...
		rkisp_params_aiisp_event_v351s(params_vdev, ISP3X_OUT_FRM_QUARTER);
		return -EINVAL;
	}
...
static void
rkisp_params_aiisp_switch_v351s(struct rkisp_isp_params_vdev *params_vdev, bool on)
{
	struct rkisp_device *dev = params_vdev->dev;
	struct rkisp_isp_stats_vdev *stats_vdev = &dev->stats_vdev;
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct isp351s_isp_params_cfg *params;
	...
	if (!on) {
		...
		val = rkisp_read(dev, ISP35_AI_CTRL, false);
		val &= ~ISP35_AIISP_ST;
		val |= ISP35_AIISP_EN | ISP35_AIPRE_IIR_EN |
...
static void
rkisp_params_isr_v351s(struct rkisp_isp_params_vdev *params_vdev, u32 isp_mis)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;
	struct isp351s_isp_params_cfg *params_rec = params_vdev->isp351s_params;
	struct rkisp_device *dev = params_vdev->dev;
	u32 i, val, reg;
		...
		if (!params_vdev->rdbk_times) {
			if (!dev->is_aiisp_en && !dev->is_aiisp_stop && priv->bay3d_iir_cnt > 1) {
				...
				for (i = 0; i < dev->unite_div; i++) {
					...
					val += priv->bay3d_iir_offs[i];
					if (priv->bay3d_iir_rw_fmt == 3) {
						isp3_param_write(params_vdev, val, ISP39_LDCV_WR_ADDR, i);
						val += priv->bay3d_iir_pk_offs[i];
					}
			...
			if (priv->bay3d_wgt_cnt > 1) {
				...
				for (i = 0; i < dev->unite_div; i++) {
					...
					val += priv->bay3d_wgt_offs[i];
					isp3_param_write(params_vdev, val, ISP3X_MI_BAY3D_CUR_WR_BASE, i);
				}
			}
			if (!dev->is_aiisp_en && !dev->is_aiisp_stop && priv->aipre_cnt > 1) {
				priv->pbuf_aipre = &priv->buf_aipre[priv->aipre_idx];
				priv->aipre_cur_idx = priv->aipre_idx;
				i = (priv->aipre_idx + 1) % priv->aipre_cnt;
				priv->aipre_idx = i;

				val = priv->buf_aipre[i].dma_addr;
				isp3_param_write(params_vdev, val, ISP35_AI_PRE_NL_WR_BASE, 0);
			}
			for (i = 0; i < dev->unite_div; i++) {
	...
	if (isp_mis & ISP3X_BAY3D_FRM_END && dev->is_aiisp_en) {
		...
		if (!IS_HDR_RDBK(dev->rd_mode))
			rkisp_params_cfg_v351s(params_vdev, i + 1, RKISP_PARAMS_IMD);
	} else if (isp_mis & CIF_ISP_FRAME && !IS_HDR_RDBK(dev->rd_mode) &&
		   !params_vdev->rdbk_times && !dev->is_aiisp_en) {
		rkisp_params_cfg_v351s(params_vdev, i + 1, RKISP_PARAMS_ALL);
	}
}

static struct rkisp_isp_params_ops rkisp_isp_params_ops_tbl = {
	.save_first_param = rkisp_save_first_param_v351s,
	.clear_first_param = rkisp_clear_first_param_v351s,
	.get_param_size = rkisp_get_param_size_v351s,
	.first_cfg = rkisp_params_first_cfg_v351s,
	.disable_isp = rkisp_params_disable_isp_v351s,
	.isr_hdl = rkisp_params_isr_v351s,
	.param_cfg = rkisp_params_cfg_v351s,
	.param_cfgsram = rkisp_params_cfgsram_v351s,
	.get_meshbuf_inf = rkisp_params_get_meshbuf_inf_v351s,
	.set_meshbuf_size = rkisp_params_set_meshbuf_size_v351s,
	.free_meshbuf = rkisp_params_free_meshbuf_v351s,
	.stream_stop = rkisp_params_stream_stop_v351s,
	.fop_release = rkisp_params_fop_release_v351s,
	.check_bigmode = rkisp_params_check_bigmode_v351s,
	.info2ddr_cfg = rkisp_params_info2ddr_cfg_v351s,
	.get_aiawb_buffd = rkisp_params_get_aiawb_buffd_v351s,
	.init_bnr_buf = rkisp_params_init_bnr_buf_v351s,
	.aiisp_event = rkisp_params_aiisp_event_v351s,
	.aiisp_start = rkisp_params_aiisp_start_v351s,
	.aiisp_switch = rkisp_params_aiisp_switch_v351s,
};

int rkisp_init_params_vdev_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
	struct rkisp_isp_params_val_v351s *priv;
	int size;
	...
	size = sizeof(struct isp351s_isp_params_cfg);
	if (params_vdev->dev->hw_dev->unite)
		size *= ISP_UNITE_MAX;
	params_vdev->isp351s_params = vmalloc(size);
	if (!params_vdev->isp351s_params) {
		kfree(priv);
		return -ENOMEM;
	}
	...
	rkisp_clear_first_param_v351s(params_vdev);
	priv->buf_info_owner = 0;
	priv->buf_info_cnt = 0;
	priv->buf_info_idx = -1;
	return 0;
...
void rkisp_uninit_params_vdev_v351s(struct rkisp_isp_params_vdev *params_vdev)
{
	struct rkisp_isp_params_val_v351s *priv = params_vdev->priv_val;

	if (params_vdev->isp351s_params)
		vfree(params_vdev->isp351s_params);
	kfree(priv);
