...
static int sgm4154x_set_ichrg_curr(struct sgm4154x_device *sgm, int uA)
{
	...
	if (sgm->device_id == SGM41512S_PN_ID || sgm->device_id == SGM41512SX_PN_ID ||
	    sgm->device_id == SGM41513_PN_ID || sgm->device_id == SGM41513X_PN_ID) {
		/* 4151x family: max charge current is 3000000 uA */
		if (uA > SGM4151x_ICHRG_I_MAX_uA)
			uA = SGM4151x_ICHRG_I_MAX_uA;
		else if (uA < SGM4154x_ICHRG_I_MIN_uA)
		...
		if (uA < SGM4154x_ICHRG_I_MIN_uA)
			uA = SGM4154x_ICHRG_I_MIN_uA;
		else if ((sgm->init_data.max_ichg > 0) && (uA > sgm->init_data.max_ichg))
