...
static int rockchip_cpuclk_v2_rate_change(struct rockchip_cpuclk *cpuclk,
					  struct clk_notifier_data *ndata)
{
					  ...
					  unsigned long event, void *data)
{
	...
	if (event == PRE_RATE_CHANGE || event == POST_RATE_CHANGE)
		ret = rockchip_cpuclk_v2_rate_change(cpuclk, ndata);
...
unregister_notifier:
	clk_notifier_unregister(hw->clk, &cpuclk->clk_nb);
free_cpuclk:
	kfree(cpuclk);
unregister_clk:
